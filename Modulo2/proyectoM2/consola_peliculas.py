...
def ejecutar_encontrar_estrenos(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    print(mod.encontrar_estrenos(p1,p2,p3,p4,p5, anio))
...
def ejecutar_cuantas_peliculas_18_mas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    Parametros:
        ...
        p5 (dict): Diccionario que contiene la informacion de la pelicula 5.
    """
    print(f'El numero de peliculas para +18 son {mod.cuantas_peliculas_18_mas(p1,p2,p3,p4,p5)}')
...
def ejecutar_reagendar_pelicula(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    if pelicula is None:
        ...
        control_horario = bool(input('Ingrese si quiere control de horario (0 = False, 1 = True): '))
        if mod.reagendar_pelicula(pelicula,nueva_hora, nuevo_dia, control_horario,p1,p2,p3,p4,p5):
            print(f'Se pudo reagendar la pelicula')
        else:
            print('No se pudo reagendar la pelicula')
def ejecutar_decidir_invitar(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    if pelicula is None:
        ...
        autorizacion =  bool(input('Cuenta su invitado con la autorización de los padres? (0 = False, 1 = True)'))
        if mod.decidir_invitar(pelicula,edad_invitado, autorizacion):
            print('Si lo puede invitar')
        else: 
            print('No lo puede invitar')
